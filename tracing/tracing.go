...
var dbInstance = attribute.Key("db.instance")
var dbType = attribute.Key("db.type")
...
func (p *otelPlugin) after() gormHookFunc {
	return func(tx *gorm.DB) {
		...
		if tx.Statement.DB.Name() != "" {
			attrs = append(attrs, dbInstance.String(tx.Statement.DB.Name()))
		}
		attrs = append(attrs, dbType.String("mysql"))
